...
def parse_tmdb_search_to_cards(data, keyword: str, limit: int = 24):
    ...
    if isinstance(data, dict) and "results" in data:
        ...
        for m in raw:
            ...
            #b) if api returns already a list
...
if st.session_state.view == "home":
    ...
    # search mode(autocomplete+word math result)
    if typed.strip():
        if len(typed.strip()) < 2:
            st.caption("Type at least 2 character for suggestions.")
        else:
...
# view details
    ...
    with right:
        ...
        st.write(data.get("overview") or "No overview avilable.")
        st.markdown("</div>", unsafe_allow_html=True)
