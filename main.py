...
def get_local_idx_by_title(title: str) -> int:
    ...
    raise HTTPException(
        status_code=404, detail=f"Title not found in local dateset: '{title}'"
    )
...
def tfidf_recommend_titles(
    ...
    if df is None or tfidf_matrix is None:
        raise HTTPException(status_code=500, detail="TF-DF resources not loaded")
...
def load_pickles():
    global df, indicies_obj, tfidf_matrix, tfidf_obj, TITLE_TO_IDX
    ...
    with open(INDICES_PATH, "rb") as f:
        indicies_obj = pickle.load(f)
        
    with open(TFIDF_MATRIX_PATH, "rb") as f:
        tfidf_matrix = pickle.load(f)

    with open(TFIDF_PATH, "rb") as f:
        title_obj = pickle.load(f)
        
    TITLE_TO_IDX = build_title_to_idx_map(indicies_obj)
